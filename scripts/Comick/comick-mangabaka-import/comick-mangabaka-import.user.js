...
// @version      1.2
// @description  Import comics from MangaBaka JSON or MAL XML export
// @author       GooglyBlox
...
(function() {
    ...
    function createMangaBakaSection() {
        ...
        section.innerHTML = `
                ...
                <div class="min-w-0 flex-1">
                    <div class="flex flex-wrap items-center gap-3">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-300">Upload the exported JSON or MAL XML file from MangaBaka.</p>
                        </div>
                    </div>
                </div>
                <div class="flex flex-wrap items-center gap-3">
            ...
            <div class="mt-4 rounded-xl bg-gray-50/80 p-4 dark:bg-gray-800/40">
                ...
                <p class="mt-2 text-sm leading-6 text-gray-600 dark:text-gray-300">Choose the JSON or MAL XML export generated by MangaBaka.</p>
                <div class="mt-4 flex flex-col gap-3 lg:flex-row lg:items-end">
                    <div class="flex-1 rounded-lg bg-white px-4 py-3 dark:bg-gray-900/80">
                        <div class="items-center">
                            <input type="file" id="mangabaka-file-input" accept=".json,.xml,application/json,application/xml,text/xml" class="block text-sm file:mr-4 file:py-2 my-2 file:px-4 file:rounded file:border-0 file:text-sm file:font-semibold file:hover:cursor-pointer file:active:border-none">
                            <div class="flex space-x-3 items-center">
                                <div class="text-sm italic">Choose the .json or .xml file</div>
                            </div>
                        </div>
                    </div>
    ...
    async function processMangaBakaFile(file) {
        ...
        try {
            ...
            const normalizedEntries = parseImportFile(fileContent);
            await importFromMangaBaka(normalizedEntries);
        } catch (error) {
    ...
    async function importFromMangaBaka(normalizedEntries) {
        const elements = {
        ...
        const stats = {
            ...
            successful: 0,
            failed: 0,
            skipped: normalizedEntries.length - supportedEntries.length
        };
    ...
    function parseImportFile(content) {
        ...
        try {
            ...
            throw new Error(`Could not parse file as JSON or XML: ${error.message}`);
        }

        if (!Array.isArray(data)) {
            throw new Error('Invalid MangaBaka JSON format. Expected an array.');
        }

        return data.map(normalizeMangaBakaEntry);
    }
    ...
    function parseMalXml(content) {
        ...
        if (parseError) {
            throw new Error('Invalid MAL XML file.');
        }

        if (!doc.querySelector('myanimelist')) {
            throw new Error('Unrecognized XML file (missing <myanimelist> root).');
        }

        const mangaNodes = doc.querySelectorAll('myanimelist > manga');
        if (mangaNodes.length === 0) {
            throw new Error('No <manga> entries found in MAL XML file.');
        }

        return Array.from(mangaNodes).map(normalizeMalEntry);
    }

    function normalizeMalEntry(mangaNode) {
        const title = mangaNode.querySelector('manga_title')?.textContent.trim() || '';
        const status = mangaNode.querySelector('my_status')?.textContent.trim() || '';
        const malId = mangaNode.querySelector('manga_mangadb_id')?.textContent.trim() || null;

        const rawStatus = normalizeStatusValue(status);
        const titleCandidates = title ? [title] : [];
        const displayTitle = title || `MAL #${malId ?? 'Unknown'}`;

        return {
            raw: { malId, title, status },
            rawStatus,
