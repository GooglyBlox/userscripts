...
class GreasyforkSync {
  ...
  async getUserScripts() {
    const userData = await this.fetchJson(`https://greasyfork.org/users/${CONFIG.userId}.json`);
    return userData.scripts || [];
  }

  async getScriptDetails(scriptId) {
    const scriptData = await this.fetchJson(`https://greasyfork.org/scripts/${scriptId}.json`);
    return scriptData;
  }
  ...
  extractCategory(script) {
    ...
    if (matches.includes('instagram') || name.includes('instagram')) return 'Instagram';
...
`;
  ...
  async syncScript(script) {
    ...
    const scriptChanged = await this.writeFileIfChanged(userJsPath, scriptContent);
    const readmeChanged = await this.writeFileIfChanged(readmePath, this.generateReadme(script, category));
...
`;
  ...
  async run() {
    try {
      ...
      await fs.writeFile(
        path.join(CONFIG.baseDir, '.sync-cache.json'), 
        JSON.stringify(cacheData, null, 2)
      );

      console.log(`Sync complete. Changes detected: ${this.changes}`);
      
      if (process.env.GITHUB_ACTIONS) {
        console.log(`::set-output name=changes::${this.changes}`);
      }
